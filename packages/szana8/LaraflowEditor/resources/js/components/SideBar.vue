  ...
  <div class="sidebar bg-white h-screen border-r border-gray-400 w-16">
    <div v-for="tab in tabs" :key="tab" class="flex tab text-lg">
      <div class="dropdown inline-block relative bg-transparent pt-2 pl-5 text-purple-600">
        <button @click="setTab(tab)" :class="{ 'text-purple-600' : active === tab }" class="tooltip focus:outline-none text-black inline-flex mr-2
    ...
    <div class="inline-block w-full relative bg-transparent pt-6 mx-2">
      <span class="plus-minus">
        <button :disabled="tabs === 1" @click="removeTab()" class="tooltip text-black inline-flex items-center
                         focus:outline-none text-purple-300 hover:text-purple-500">
          <span class="mr-1"><i class="fas fa-minus" /></span>
        </button>

        <button :disabled="tabs >= 10" @click="addTab()" class="tooltip text-black inline-flex items-center
                     focus:outline-none text-purple-300 hover:text-purple-500">
  ...
  methods: {
    ...
    removeTab() {
      Object.keys(localStorage)
        .filter(key => {
          return (
            key.indexOf("schematics-settings") === 0 &&
